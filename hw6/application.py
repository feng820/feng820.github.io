...
def get_stock_summary(ticker):
    ...
    if len(data) > 0:
        ...
        tabular_data = {'ticker': data.get('ticker'),
                        ...
                        'prevClose': round(prev_close_price, 2) if prev_close_price != 0 else '',
                        'open': round(data.get('open'), 2) if data.get('open') else '',
                        'high': round(data.get('high'), 2) if data.get('high') else '',
                        'low': round(data.get('low'), 2) if data.get('low') else '',
                        'last': round(last_price, 2) if last_price != 0 else '',
